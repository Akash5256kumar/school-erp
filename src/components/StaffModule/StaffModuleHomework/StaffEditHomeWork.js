...
const StaffEditHomeWork = props => {
  ...
  const [publishDate, setPublishDate] = useState(new Date(homeWork?.as_date));
  const [modalVisible, setModalVisible] = useState(false);
  const [selectedImage, setSelectedImage] = useState(null);
  const [stdClass, setStdClass] = useState(homeWork?.std_class || homeWork?.class || null);
  const [section, setSection] = useState(homeWork?.std_section || homeWork?.section || null);
  const [subject, setSubject] = useState(homeWork?.subject || null);
  const [book, setBook] = useState(homeWork?.book || '');
  const [topic, setTopic] = useState(homeWork?.topic || '');
  const [chapter, setChapter] = useState(homeWork?.chapter || '');
  ...
  const updateHomework = async () => {
    ...
    try {
      ...
      if (response?.status) {
        showSuccessToast({
          message: 'The homework has been updated successfully.',
      ...
      console.log('Edit Homework Error:', error);
